...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		if(![newSocket attachStreamsToRunLoop:runLoop error:nil] || ![newSocket configureStreamsAndReturnError:nil] || ![newSocket openStreamsAndReturnError:nil]) {
            [newSocket close];
            return;
        }
...
- (NSData *)connectedAddress
{
    ...
    if (theSocket)
    {
		...
		return (__bridge_transfer NSData *)peeraddr;
	}
...
- (NSData *)localAddress
{
    ...
    if (theSocket)
    {
		...
		return (__bridge_transfer NSData *)selfaddr;
	}
